...
static void draw_gauge(lv_layer_t *layer, const struct status_state *state) {
    ...
    layer_draw_image(layer, CONFIG_NICE_OLED_WIDGET_WPM_GAUGE_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_WPM_GAUGE_CUSTOM_Y, &gauge, &img_dsc);
}
...
static void draw_grid(lv_layer_t *layer) {
    ...
    layer_draw_image(layer, 0, 65, &grid, &img_dsc);
}
...
#define DRAW_LABEL_FONT_SIZE 14
...
#define DRAW_LABEL_FONT_SIZE 16
...
#define DRAW_LABEL_WMP_X 26
    ...
    layer_draw_label(layer, 0, DRAW_LABEL_WMP_Y, 25, DRAW_LABEL_FONT_SIZE, &label_left_dsc, DRAW_LABEL_WMP);
    ...
    layer_draw_label(layer, DRAW_LABEL_WMP_X, DRAW_LABEL_WMP_Y, 42, DRAW_LABEL_FONT_SIZE, &label_dsc_wpm, wpm_text);
}
...
static void draw_grid(lv_layer_t *layer) {
    ...
    layer_draw_image(layer, -1, 95, &grid, &img_dsc);
}
...
static void draw_label(lv_layer_t *layer, const struct status_state *state) {
    ...
    if (state->wpm[9] < 10) {
        layer_draw_label(layer, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_X + 5, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_Y, 50, 12, &label_dsc_wpm, wpm_text);
    } else if (state->wpm[9] >= 10 && state->wpm[9] < 100) {
        layer_draw_label(layer, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_X + 2, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_Y, 50, 12, &label_dsc_wpm, wpm_text);
    } else {
        layer_draw_label(layer, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_WPM_LABEL_CUSTOM_Y, 50, 12, &label_dsc_wpm, wpm_text);
    }
}
#endif // IS_ENABLED(CONFIG_NICE_OLED_WIDGET_WPM_NUMBER)
